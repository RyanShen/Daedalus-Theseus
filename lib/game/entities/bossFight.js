...
EntityBossFight = ig.Entity.extend({
    ...
    init: function(x,y,settings){
        ...
        ig.game.spawnEntity(EntityProblemtrigger, 230 + ig.game.screen.x, 200 + ig.game.screen.y, {name: 'q14', distance: -1, key: 1, eventID: 50}); // body top left
        ig.game.spawnEntity(EntityProblemtrigger, 250 + ig.game.screen.x, 631 + ig.game.screen.y, {name: 'q15', distance: -1, key: 1, eventID: 50}); // body bottom left
        ig.game.spawnEntity(EntityProblemtrigger, 755 + ig.game.screen.x, 195 + ig.game.screen.y, {name: 'q16', distance: -1, key: 1, eventID: 50}); // body top right
        ig.game.spawnEntity(EntityProblemtrigger, 800 + ig.game.screen.x, 625 + ig.game.screen.y, {name: 'q17', distance: -1, key: 1, eventID: 50}); // body bottom right
        ig.game.spawnEntity(EntityProblemtrigger, 539 + ig.game.screen.x, 150 + ig.game.screen.y, {name: 'q18', distance: -1, key: 1, eventID: 50}); // body part middle
        q14 = ig.game.getEntityByName('q14');
        q15 = ig.game.getEntityByName('q15');
        q16 = ig.game.getEntityByName('q16');
        q17 = ig.game.getEntityByName('q17');
        q18 = ig.game.getEntityByName('q18');
    ...
    update: function(){
        ...
        q14.pos = {x: 230 + ig.game.screen.x, y: 200 + ig.game.screen.y};
        q15.pos = {x: 250 + ig.game.screen.x, y: 631 + ig.game.screen.y};
        q16.pos = {x: 755 + ig.game.screen.x, y: 195 + ig.game.screen.y};
        q17.pos = {x: 800 + ig.game.screen.x, y: 625 + ig.game.screen.y};
        q18.pos = {x: 539 + ig.game.screen.x, y: 150 + ig.game.screen.y};
        ...
        if (!qaInterface.isActive) {
            if (q14.solved && q14.eventTriggered && this.stateBodyParts[1] == 'alive') {
                this.destroyBodyParts(1);
                q14.problemTriggerLock = true;
            }
            else if (q14.solved && this.stateBodyParts[1] == 'dead') {
                q14.problemTriggerLock = true;
            }
            if (q15.solved && q15.eventTriggered && this.stateBodyParts[2] == 'alive') {
                this.destroyBodyParts(2);
                q15.problemTriggerLock = true;
            }
            else if (q15.solved && this.stateBodyParts[2] == 'dead') {
                q15.problemTriggerLock = true;
            }
            if (q16.solved && q16.eventTriggered && this.stateBodyParts[3] == 'alive') {
                this.destroyBodyParts(3);
                q16.problemTriggerLock = true;
            }
            else if (q16.solved && this.stateBodyParts[3] == 'dead') {
                q16.problemTriggerLock = true;
            }
            if (q17.solved && q17.eventTriggered && this.stateBodyParts[4] == 'alive') {
                this.destroyBodyParts(4);
                q17.problemTriggerLock = true;
            }
            else if (q17.solved && this.stateBodyParts[4] == 'dead') {
                q17.problemTriggerLock = true;
            }
            if (q18.solved && q18.eventTriggered &&this.stateBodyParts[0] == 'alive') {
                this.destroyBodyParts(0);
                q18.problemTriggerLock = true;
            }
            else if (q18.solved && this.stateBodyParts[0] == 'dead') {
                q18.problemTriggerLock = true;
            }
        }
    ...
    destroyBodyParts: function( num ){
        ...
        if (q18.locked && ig.game.getEntitiesByType('EntityQuestionController')[0].isSolved(2, 14, 17)) {
            if (!ig.game.getEntitiesByType(EntityQuestions)[0].isActive) {
                q18.unlock();
            }
        }
