...
MyGame = ig.Game.extend({
	...
	init: function() {
        ...
        //question interface
        ig.input.bind( ig.KEY.P, 'generate' );
        ig.input.bind( ig.KEY.L, 'generate2' );
        ig.input.bind( ig.KEY.M, 'generate3' );

        this.levelController.loadNewGameLevel();
        this.setupCamera();
        ig.qs = this.spawnEntity( EntityQuestions, 0, 0 );
	...
	update: function() {
        ...
        if (ig.input.pressed('generate')){
                if(!ig.qs.isActive){
                    ...
                    ig.qs.ProblemDisplay(1);
                }
                else{
                    this.player.setMovementLock(false);
                    ig.qs.ProblemHide();
                }
        }else if(ig.input.pressed('generate2')){
                if(!ig.qs.isActive){
                    this.player.setMovementLock(true);
                    ig.qs.ProblemDisplay(2);
                }
                else{
                    this.player.setMovementLock(false);
                    ig.qs.ProblemHide();
        ...
        else if(ig.input.pressed('generate3')){
            if(!ig.qs.isActive){
                this.player.setMovementLock(true);
                ig.qs.ProblemDisplay(3);
            }
            else{
                this.player.setMovementLock(false);
                ig.qs.ProblemHide();
