...
.defines(function(){
    ...
    DialogController = ig.Class.extend({
        ...
        init: function() {
            ...
            this.dialogSetJSON= [
                {id: 1, content: [
                    { name: this.avatar[ig.game.avatarID].name, side: 'left' , mediaNum: 1, text: 'Hmm I managed to fix it. Maybe I could put into Dad\'s PC later.' }
                ]},
                {id: 2, content: [
                    { name: this.avatar[ig.game.avatarID].name, side: 'left' , mediaNum: 1, text: 'An encrypted text. Seems like Dad is hiding a password in it. It might be a password for his computer.' }
                ]},
                {id: 3, content: [
