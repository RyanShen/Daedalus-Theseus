...
EntityProblemtrigger = ig.Entity.extend({
	...
	init: function(x, y, settings) {
		...
		this.problemType = this.getProblemInfo();
		this.problemScore = this.getProblemScore();
		this.getImageID();
	...
	activate: function() {
		...
		if (!this.eventTriggered && !this.solved && !this.qaInterface.isActive) {
	     ...
	     if (!this.eventTriggered && this.solved) {
	     	this.eventTriggered = true;
	     }

	     if (this.eventTriggered && this.solved && !this.qaInterface.isActive) {
	     	if(this.nextQuestion != null) {
	     		for(var i = 1; i < nextQuestion.length; i+=2) {
	     			var nextItem = nextQuestion.charAt(i) * 10 + nextQuestion.charAt(i+1);
	...
	checkSolve: function() {
		var requestURL = "http://128.2.238.182:3000/team?tid=4";
         $.ajax({
             type:'GET',
             url: requestURL,
             async: false,
             ...
             dataType: 'json',
             success: function(data) {
                 var problemSolved = data.problemSolved;
                 for (var i in problemSolved) {
                 	if (problemSolved[i] == this.problemID){
                 		return true;
                 	}
                 }
                 return false;
