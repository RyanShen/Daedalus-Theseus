...
MyGame = ig.Game.extend({
	...
	init: function() {
        ...
        this.avatarName = this.dataLoader.getAvatarName();
    ...
    toggleUIPlayerInteraction: function( toggle ){
        ...
        if( toggle )
        {
            ...
            if( player )
                player.movementLock = false;
            this.inGameGUIController.toggleUI( true );
        }
        else
        {
            var player = ig.game.getEntitiesByType(EntityPlayer)[0];
            if( player )
                player.movementLock = true;
            this.inGameGUIController.toggleUI( false );
        }
    },
    ...
    toggleSubsystemsInteraction: function( toggle ) {
        if( !toggle )
        {
            ...
            var boss = ig.game.getEntitiesByType(EntityBoss);
            if( boss )
                boss.isSuspended = true;
            var bossPlat = ig.game.getEntitiesByType(EntityBossPlatform);
            if( bossPlat )
            ...
            var dialogbox = ig.game.getEntitiesByType(EntityDialogBox);
            if( dialogbox )
                dialogbox.isSuspended = true;
            var dialogplayer = ig.game.getEntitiesByType(EntityDialogPlayer);
            if( dialogplayer )
