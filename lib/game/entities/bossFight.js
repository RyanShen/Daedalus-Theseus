...
EntityBossFight = ig.Entity.extend({
    ...
    init: function(x,y,settings){
        ...
        if( !ig.gui.element.action('getByName', 'BOSS_Button_Close') )
        {
            ig.gui.element.add({
                ...
                click: function() {
                    if( ig.game.getEntitiesByType(EntityBossFight)[0] )
        ...
        ig.game.spawnEntity(EntityProblemtrigger, 200 + ig.game.screen.x, 150 + ig.game.screen.y, {name: 'q19', distance: -1, key: 1, eventID: 50}); // body top left
        ig.game.spawnEntity(EntityProblemtrigger, 80 + ig.game.screen.x, 550 + ig.game.screen.y, {name: 'q20', distance: -1, key: 1, eventID: 50}); // body bottom left
        ig.game.spawnEntity(EntityProblemtrigger, 760 + ig.game.screen.x, 150 + ig.game.screen.y, {name: 'q21', distance: -1, key: 1, eventID: 50}); // body top right
        ig.game.spawnEntity(EntityProblemtrigger, 880 + ig.game.screen.x, 550 + ig.game.screen.y, {name: 'q22', distance: -1, key: 1, eventID: 50}); // body bottom right
        ig.game.spawnEntity(EntityProblemtrigger, 480 + ig.game.screen.x, 120 + ig.game.screen.y, {name: 'q23', distance: -1, key: 1, eventID: 50}); // body part middle
    ...
    update: function(){
        //console.log("q1 " + q1.problemTriggerLock + " q2 " + q2.problemTriggerLock + " q3 " + q3.problemTriggerLock + " q4 " + q4.problemTriggerLock + " q5 " + q5.problemTriggerLock);
        if( this.isSuspended )
            return;
        ...
        if (this.deathTimer && this.deathTimer.delta() > 0) {
            ...
            if(ig.gui.element.action('getByName', 'BOSS_Button_Close'))
            {
                ig.gui.element.action('remove', 'BOSS_Button_Close');
            }
        ...
        q1.pos = {x: 200 + ig.game.screen.x, y: 150 + ig.game.screen.y};
        q2.pos = {x: 80 + ig.game.screen.x, y: 550 + ig.game.screen.y};
        q3.pos = {x: 760 + ig.game.screen.x, y: 150 + ig.game.screen.y};
        q4.pos = {x: 880 + ig.game.screen.x, y: 550 + ig.game.screen.y};
        q5.pos = {x: 480 + ig.game.screen.x, y: 120 + ig.game.screen.y};
