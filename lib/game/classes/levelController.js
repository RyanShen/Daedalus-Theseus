    ...
    ig.LevelController = ig.Class.extend({
        ...
        loadLevelfromSave: function() {
            if (this.isValid(ig.game.level)) {
                ig.game.eventController.loadEventsFromSave(ig.game.eventID);
                ig.game.spawnEntity(EntityPlayer, ig.game.pos.x, ig.game.pos.y);
                this.setupCamera();
        ...
        isValid: function(levelName) {
            switch(levelName) {
                ...
                case '3_Tower_TopFloor':
                    return true;
                case 'level2':
                    ig.game.level = '2_PoliceHQ_GroundFloor';
                    return true;
                default :
                    return false;
            }
