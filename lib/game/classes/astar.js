...
AStar = ig.Class.extend({
    ...
    searchPath: function( start, end, gridObj ){
        ...
        //refresh lists and grid data for new astar search
        gridObj.refresh();
        var grid = gridObj.gridData;

        //end point modification
        //if end is a blocked tile, find a near non-blocked tile as the actual end
        if( ig.game.collisionMap.data[end.y][end.x] != 0 )
        {

            var neighbor = this.getNeighborNodes( end, grid, 1 );
            var newEnd = null;
            var distance = this.heuristic( start.x, start.y, end.x, end.y );

            for( var i = 0; i< neighbor.length; i++ )
            {
                if( ig.game.collisionMap.data[neighbor[i].y][neighbor[i].x] != 0 )//blocked
                    continue;

                var newDis = this.heuristic( start.x, start.y, neighbor[i].x, neighbor[i].y );
                if( newDis < distance )
                {
                    distance = newDis;
                    newEnd = neighbor[i];
            ...
            {
                neighbor = this.getNeighborNodes( end, grid, 2 );
                for( var i = 0; i< neighbor.length; i++ )
                {
                    if( ig.game.collisionMap.data[neighbor[i].y][neighbor[i].x] != 0 )//blocked
                        continue;
        ...
        while( openNodes.length > 0 ) {
            ...
            var neighborNodes = this.getNeighborNodes( currentNode, grid, 1 );
    ...
    getNeighborNodes: function( node, grid, distance ){
    ...
    heuristic: function( x1, y1, x2, y2 ) {
        ...
        return this.diagonalDistance( x1, y1, x2, y2 );
    },
